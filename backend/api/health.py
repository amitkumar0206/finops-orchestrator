...
Security:
- /health, /health/liveness, /health/readiness are public lightweight probes.
  They never reveal infrastructure details or raw exception strings.
- /health/detailed requires JWT authentication and returns full service status.
  Even here, raw exception text is logged but never included in the response.
"""
...
# ---------------------------------------------------------------------------
# Public probes — no infrastructure details ever exposed
# ---------------------------------------------------------------------------

@router.get("")
@router.get("/")
async def health_check():
    """Public health probe. Returns status only — no internal details."""
...
# ---------------------------------------------------------------------------
# Authenticated detailed health — full service status
# ---------------------------------------------------------------------------

@router.get("/detailed", response_model=HealthCheck)
async def detailed_health(request: Request, user=Depends(require_auth)) -> HealthCheck:
    """
    Comprehensive health check for all system components.

    Requires authentication.  Raw exception strings are written to structured
    logs only — they are never included in the HTTP response.
    """
...
# All helpers log errors via structlog and return only safe generic messages.
# ---------------------------------------------------------------------------
...
async def _check_llm_services() -> Dict[str, Any]:
    ...
    try:
        ...
        settings = get_settings()
        start = time.time()
        bedrock = create_aws_client(
            AwsService.BEDROCK_RUNTIME,
            region_name=getattr(settings, "aws_region", DEFAULT_AWS_REGION),
        )
...
async def _check_aws_services() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            logger.error("athena_health_check_failed", error=str(e))
        ...
        try:
            ...
            details["s3_bucket"] = "error: service unavailable"
            all_healthy = False
        ...
        try:
            ...
            logger.error("cur_data_health_check_failed", error=str(e))
            details["cur_data"] = "error: service unavailable"
            all_healthy = False
        ...
        try:
            ...
            details["athena_database"] = "available"
        except ClientError as e:
            logger.error("athena_database_health_check_failed", error=str(e))
            details["athena_database"] = "error: not found"
            all_healthy = False
        ...
        try:
            ...
            for _ in range(10):
                ...
                if state == 'SUCCEEDED':
                    details["athena_table"] = "queryable"
                    details["cur_table_health"] = "healthy"
                    break
                elif state in ['FAILED', 'CANCELLED']:
                    logger.error(
                        "athena_table_health_check_failed",
                        state=state,
                        reason=status_response['QueryExecution']['Status'].get(
                            'StateChangeReason', 'Unknown'
                        ),
                    )
                    details["athena_table"] = "error: query failed"
                    details["cur_table_health"] = "unhealthy"
                    all_healthy = False
                    break
            else:
                details["athena_table"] = "error: query timeout"
                details["cur_table_health"] = "unknown"

        except Exception as e:
            logger.error("athena_table_health_check_failed", error=str(e))
            details["athena_table"] = "error: service unavailable"
            details["cur_table_health"] = "unhealthy"
            all_healthy = False

        # Check Cost Explorer (optional — does not affect overall status)
        try:
            ...
            logger.error("cost_explorer_health_check_failed", error=str(e))
            details["cost_explorer"] = "error: service unavailable"
        ...
        logger.error("aws_services_health_check_failed", error=str(e), exc_info=True)
        return {
            "status": "unhealthy",
