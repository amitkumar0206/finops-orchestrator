...
# aasmaa AI Cost Intelligence Platform - Deployment Script
...
save_deployment_state() {
    ...
    if [ ! -f "$ENV_FILE" ] || ! grep -q "# DEPLOYMENT STATE" "$ENV_FILE"; then
        cat > "$ENV_FILE" << 'EOF'
# aasmaa Intelligence Platform - Configuration
...
#   aasmaa-athena-results-<AWS_ACCOUNT_ID>
# Example:
# ATHENA_RESULTS_BUCKET=aasmaa-athena-results-123456789012
...
detect_existing_infrastructure() {
    ...
    if aws athena get-work-group --work-group aasmaa-workgroup --region "$AWS_REGION" &>/dev/null; then
        log_success "Found existing Athena workgroup: aasmaa-workgroup"
        athena_exists=true
    else
...
setup_one_time_components() {
    ...
    if [ -z "$S3_BUCKET" ]; then
        ...
        S3_BUCKET="aasmaa-data-${AWS_ACCOUNT_ID}"
    fi
    ...
    if [ -z "$ATHENA_RESULTS_BUCKET" ]; then
        ATHENA_RESULTS_BUCKET="aasmaa-athena-results-${AWS_ACCOUNT_ID}"
    fi
...
deploy_infrastructure() {
    ...
    echo "1) Amazon Nova Pro 2.0 (Default - Best for production aasmaa analysis)"
    ...
    if [ -n "$qid" ]; then
        ...
        if aws athena get-query-results --query-execution-id "$qid" --region "$AWS_REGION" 2>/dev/null | grep -q "${table_name}"; then
            ...
            if [ "$glue_tables_count" -le 2 ]; then
                ...
                local cur_s3_path="s3://${S3_BUCKET}/${CUR_S3_PREFIX:-cost-exports/aasmaa-cost-export}/"
...
deploy_services() {
    ...
    if [ -n "${CUR_BUCKET:-}" ]; then
        ECS_SERVICE_PARAMS+=(
            ...
            CurS3Prefix="${CUR_S3_PREFIX:-cost-exports/aasmaa-cost-export}"
        )
    else
        ...
        ECS_SERVICE_PARAMS+=(
            ...
            CurTable="${AWS_CUR_TABLE:-cur_data}"
            CurS3Prefix="cost-exports/aasmaa-cost-export"
        )
    fi
...
get_deployment_info() {
    ...
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  📊 aasmaa AI Cost Intelligence Platform"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    ...
    if [ "$CUR_CONFIGURED" = "true" ]; then
        ...
        echo "   → Settings:"
        echo "      • Report name: aasmaa-cost-report"
        ...
        echo "      • Report path prefix: cur/aasmaa-cost-report"
        echo ""
        ...
        echo "   → Verify data: aws s3 ls s3://$S3_BUCKET/cur/aasmaa-cost-report/"
        echo ""
...
validate_deployment() {
    ...
    DB_LOGS=$(aws logs tail /ecs/aasmaa/backend --since 2m --region "$AWS_REGION" --format short 2>/dev/null | grep -i "database" | tail -5 || echo "")
...
print_deployment_summary() {
    ...
    if aws ecr describe-repositories --repository-names aasmaa-backend --region "$AWS_REGION" >/dev/null 2>&1; then
        BACKEND_ECR="EXISTS"
    else
        BACKEND_ECR="NOT_FOUND"
    fi
    if aws ecr describe-repositories --repository-names aasmaa-frontend --region "$AWS_REGION" >/dev/null 2>&1; then
        FRONTEND_ECR="EXISTS"
    else
    ...
    if aws athena get-work-group --work-group aasmaa-workgroup --region "$AWS_REGION" >/dev/null 2>&1; then
...
print_destroy_summary() {
    ...
    BACKEND_ECR_EXISTS=$(aws ecr describe-repositories --repository-names aasmaa-backend --region "$AWS_REGION" 2>/dev/null && echo "STILL_EXISTS" || echo "DELETED")
    FRONTEND_ECR_EXISTS=$(aws ecr describe-repositories --repository-names aasmaa-frontend --region "$AWS_REGION" 2>/dev/null && echo "STILL_EXISTS" || echo "DELETED")
    
    # Check what was preserved
    CUR_EXISTS=$(aws cur describe-report-definitions --region us-east-1 --query 'ReportDefinitions[?contains(ReportName, `aasmaa`)]' --output text 2>/dev/null | wc -l | tr -d ' ')
    S3_BUCKETS=$(aws s3api list-buckets --query "Buckets[?contains(Name, 'aasmaa')].Name" --output text 2>/dev/null | wc -w || echo "0")
    GLUE_DB_EXISTS=$(aws glue get-database --name cost_usage_db --region "$AWS_REGION" 2>/dev/null && echo "PRESERVED" || echo "DELETED")
    ATHENA_WG_EXISTS=$(aws athena get-work-group --work-group aasmaa-workgroup --region "$AWS_REGION" 2>/dev/null && echo "PRESERVED" || echo "DELETED")
    ...
    if [ "$S3_BUCKETS" -gt 0 ]; then
        ...
        aws s3api list-buckets --query "Buckets[?contains(Name, 'aasmaa')].Name" --output text 2>/dev/null | tr '\t' '\n' | sed 's/^/    - /'
    else
...
print_destroyAll_summary() {
    ...
    BACKEND_ECR_EXISTS=$(aws ecr describe-repositories --repository-names aasmaa-backend --region "$AWS_REGION" 2>/dev/null && echo "STILL_EXISTS" || echo "DELETED")
    FRONTEND_ECR_EXISTS=$(aws ecr describe-repositories --repository-names aasmaa-frontend --region "$AWS_REGION" 2>/dev/null && echo "STILL_EXISTS" || echo "DELETED")
    CUR_COUNT=$(aws cur describe-report-definitions --region us-east-1 --query 'ReportDefinitions[?contains(ReportName, `aasmaa`)]' --output text 2>/dev/null | wc -l | tr -d ' ')
    S3_BUCKET_COUNT=$(aws s3api list-buckets --query "Buckets[?contains(Name, 'aasmaa')].Name" --output text 2>/dev/null | wc -w || echo "0")
    GLUE_DB_EXISTS=$(aws glue get-database --name cost_usage_db --region "$AWS_REGION" 2>/dev/null && echo "STILL_EXISTS" || echo "DELETED")
    ATHENA_WG_EXISTS=$(aws athena get-work-group --work-group aasmaa-workgroup --region "$AWS_REGION" 2>/dev/null && echo "STILL_EXISTS" || echo "DELETED")
    ...
    GLUE_ROLE_EXISTS=$(aws iam get-role --role-name "AWSGlueServiceRole-aasmaa" 2>/dev/null && echo "STILL_EXISTS" || echo "DELETED")
    ...
    if [ "$FAILED_COUNT" -eq 0 ]; then
        ...
        log_success "All aasmaa resources have been permanently deleted."
        echo ""
        ...
        if [ "$CUR_COUNT" -gt 0 ]; then
            ...
            aws cur describe-report-definitions --region us-east-1 --query 'ReportDefinitions[?contains(ReportName, `aasmaa`)].ReportName' --output text 2>/dev/null | tr '\t' '\n' | while read report; do
...
destroy_infrastructure() {
    ...
    echo "  • ECR repositories: aasmaa-backend, aasmaa-frontend"
    ...
    aws ecr delete-repository --repository-name aasmaa-backend --force --region "$AWS_REGION" 2>/dev/null || true
    aws ecr delete-repository --repository-name aasmaa-frontend --force --region "$AWS_REGION" 2>/dev/null || true
...
destroy_all() {
    ...
    if aws ecr describe-repositories --repository-names aasmaa-backend --region "$AWS_REGION" &>/dev/null; then
        RESOURCES_TO_DELETE+=("  ✓ ECR repository: aasmaa-backend")
    fi
    if aws ecr describe-repositories --repository-names aasmaa-frontend --region "$AWS_REGION" &>/dev/null; then
        RESOURCES_TO_DELETE+=("  ✓ ECR repository: aasmaa-frontend")
    fi
    
    # Check for Data Exports (new API)
    DATA_EXPORTS=$(aws bcm-data-exports list-exports --region us-east-1 --query "Exports[?contains(Export.Name, 'aasmaa')].Export.Name" --output text 2>/dev/null || echo "")
    ...
    CUR_REPORTS=$(aws cur describe-report-definitions --region us-east-1 --query "ReportDefinitions[?contains(ReportName, 'aasmaa')].ReportName" --output text 2>/dev/null || echo "")
    ...
    S3_BUCKETS=$(aws s3api list-buckets --query "Buckets[?contains(Name, 'aasmaa')].Name" --output text 2>/dev/null || echo "")
    ...
    GLUE_CRAWLERS=$(aws glue list-crawlers --region "$AWS_REGION" --query "CrawlerNames[?contains(@, 'aasmaa')]" --output text 2>/dev/null || echo "")
    ...
    ATHENA_WORKGROUPS=$(aws athena list-work-groups --region "$AWS_REGION" --query "WorkGroups[?contains(Name, 'aasmaa')].Name" --output text 2>/dev/null || echo "")
    ...
    if [ ${#RESOURCES_TO_DELETE[@]} -eq 0 ]; then
        log_info "No aasmaa resources found to delete."
        exit 0
    fi
    ...
    log_info "Checking for aasmaa IAM roles..."
    ...
    AASMAA_VPCS=$(aws ec2 describe-vpcs --region "$AWS_REGION" \
        --filters "Name=tag:Name,Values=*${STACK_NAME}*" \
        --query 'Vpcs[].VpcId' --output text 2>/dev/null || echo "")
    
    if [ -n "$AASMAA_VPCS" ]; then
        log_info "Found VPCs to clean up: $AASMAA_VPCS"
        
        for vpc in $AASMAA_VPCS; do
...
main() {
    log_info "Starting aasmaa AI Cost Intelligence Platform deployment..."
